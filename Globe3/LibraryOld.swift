...
class LibraryControllerOld : ControllerAvecMenu, UIScrollViewDelegate {
    ...
    func makeHV(ud : NSUserDefaults) {
        hv = HelpView(text: "From the Library you can easily explore all that globe has to offer, from gripping thrillers and mind-bending mystery to dreamy romance and timeless classics.\n\nWe're contantly expanding our collection and adding new titles so be sure to check in regularly.\n\nTap 'more' to find see more from that genre, or tap the search icon to search for a title", arrow: true, origin: PERC_LibHeaderHeight + 69)
        hv.center.x = self.view.center.x
        UIApplication.sharedApplication().keyWindow!.addSubview(hv)
        hv.setBold(NSMakeRange(9, 7))
        hv.setBold(NSMakeRange(74, 18))
        hv.setBold(NSMakeRange(97, 20))
        hv.setBold(NSMakeRange(121, 14))
        hv.setBold(NSMakeRange(140, 17))
        hv.setBold(NSMakeRange(263, 6))
        hv.setBold(NSMakeRange(329, 18))
    ...
    func buildViews() {
        ...
        let vLab = UIView(frame: CGRect(x: PERC_BlackWidth, y: PERC_BlueHeight + CGRectGetMaxY(ivHeader.frame), width: AlmostFullWidth, height: PERC_TitlePurpleHeight))
        let lTitle = UILabel()
        lTitle.text = "CLASSIC"
        ...
        let bMore = UIButton()
        bMore.setTitle("MORE", forState: .Normal)
        bMore.sizeToFit()
        ...
        bMore.addTarget(self, action: #selector(LibraryController.classicMoreClicked(_:)), forControlEvents: .TouchUpInside)
        ...
        scFeatured.tag = getTagFromCat("Classic")
        searchBooks("Classic", sv: scFeatured, latest: false)//GETTING THE BOOKS HERE
        ...
        scLatest.tag = getTagFromCat("Classic")
        searchBooks("Classic", sv: scLatest, latest: true)//GETTING THE BOOKS HERE
        ...
        var originY = CGRectGetMaxY(vContainer.frame)
        
        for like in self.arrLike {
            if (like as! String) != "Classic" {
                ...
                let lTitle = UILabel()
                lTitle.text = getCatTitle(like as! String).uppercaseString
                lTitle.font = titleFont
                lTitle.sizeToFit()
                lTitle.frame.size.height = PERC_TitlePurpleHeight
                lTitle.frame.origin = CGPoint(x: 10, y: 0)
                lTitle.textColor = UIColor.whiteColor()
                vLab.addSubview(lTitle)
                vLab.backgroundColor = nextColorIsBlue ? colClassicBlueUnavailable : colClassicRedUnavailable
                nextColorIsBlue = !nextColorIsBlue
                originY = CGRectGetMaxY(vLab.frame)
                scMain.addSubview(vLab)
            }
        }
        
        for bof in arrWhatever {
            if (bof as! String) != "Classic" {
                let vLab = UIView(frame: CGRect(x: PERC_BlackWidth, y: PERC_BlueHeight + originY, width: AlmostFullWidth, height: PERC_TitlePurpleHeight))
                let lTitle = UILabel()
                lTitle.text = getCatTitle(bof as! String).uppercaseString
                lTitle.font = titleFont
                lTitle.sizeToFit()
                lTitle.frame.size.height = PERC_TitlePurpleHeight
                lTitle.frame.origin = CGPoint(x: 10, y: 0)
        ...
        for hate in arrHate {
            if (hate as! String) != "Classic" {
                let vLab = UIView(frame: CGRect(x: PERC_BlackWidth, y: PERC_BlueHeight + originY, width: AlmostFullWidth, height: PERC_TitlePurpleHeight))
                let lTitle = UILabel()
                lTitle.text = getCatTitle(hate as! String).uppercaseString
                lTitle.font = titleFont
                lTitle.sizeToFit()
                lTitle.frame.size.height = PERC_TitlePurpleHeight
                lTitle.frame.origin = CGPoint(x: 10, y: 0)
    ...
    @IBAction func classicMoreClicked(sender : UIButton) {
