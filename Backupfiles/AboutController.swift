...
class AboutController : UIViewController, MFMailComposeViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        /*let txtHow = "Reading should be a right, not a privilege. If you agree then you can rest assured you’re in good company. At globe we think that everyone should be able to access great bestselling books. We also believe writers and authors should get paid for their work.\n\nBy introducing non-intrusive digital advertising onto our eReading platform we offer users free access to the top bestsellers across fiction and non-fiction. This not only allows you to read the books you love free of charge, but lets you directly support your favourite authors simply by reading their book. The more free books you read, the more they receive; it’s as simple as that.\n\nAs well as giving you free access to the good and the great, globe curates a more tailored reading experience by showing you more of the books you want and less of the ones you don’t. Want to indulge in more of your favourite genres or explore a completely something completely new, globe makes it easier than ever to discover your next great literary love."*/
        ...
        /*var txtContact = ""
        var asContact = NSAttributedString(string: txtContact, attributes: [NSParagraphStyleAttributeName: paraStyle,
            NSBaselineOffsetAttributeName: NSNumber(float: 0)]).length
        lContact.textAlignment = NSTextAlignment.Justified
        lContact.sizeToFit()
        lContact.frame.size.width = AlmostFullWidth
        lContact.frame.origin.y = CGRectGetMaxY(bContact.frame) + PERC_BlueHeight
        lContact.frame.origin.x = PERC_BlackWidth
        lContact.frame.size.height = 0*/
        
        vContact.frame.size.width = AlmostFullWidth
        vContact.frame.size.height = onePxHeight
        vContact.frame.origin.x = PERC_BlackWidth
        //vContact.frame.origin.y = CGRectGetMaxY(lContact.frame) + PERC_BlueHeight
        ...
        for v in scMain.subviews {
            contentRect = CGRectUnion(contentRect, v.frame);
        }*/
